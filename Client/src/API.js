      ...
      .then((response) => {
        if (response.ok) {
         ...
         response.json()
            ...
            .catch( err => reject({ error: "Cannot parse server response 1" }))
        } else {
          ...
          response.json()
            ...
            .catch(err => reject({ error: "Cannot parse server response 2" })) // something else
        }
      })
      .catch(err => 
...
// ------------------------ NEW ------------------------
/**
 * Getting from the server side and returning the list of ALL films that the current user is a reviewr of.
 */
const getAllPublicFilmsToReview = async () => {
  // film.watchDate could be null or a string in the format YYYY-MM-DD
  var path = SERVER_URL + 'films/public/all';
  return getJson(fetch(path, { credentials: 'include' })).then( json => { 
    const returnFilms = json.films.map((film) => new Film(film));
    return returnFilms;
  })
}
// -----------------------------------------------------
 ...
 */
 const getFilmReviews = async (filmId, pageNumber) => {
  ...
  if(pageNumber != undefined) path += '?pageNo=' + pageNumber;
  return getJson(fetch(path, { credentials: 'include' }))
    .then( json => { 
      localStorage.setItem('totalPages',  json.totalPages);
      localStorage.setItem('currentPage', json.currentPage);
      localStorage.setItem('totalItems',  json.totalItems);
...
const getFilm = async (filmId) => {
  return getJson( fetch(SERVER_URL + 'films/' + localStorage.getItem('filmsType') + '/'+ String(filmId), { credentials: 'include' }))
...
const API = {logIn, getUserInfo, getUsers, logOut, getPrivateFilms, getPublicFilms, getFilmReviews, updateFilm, deleteFilm, addFilm, getFilm, getFilters, issueReview, deleteReview, getReview, updateReview, getPublicFilmsToReview, getAllPublicFilmsToReview, selectFilm};
export default API;
