...
function FilmReviewTable(props) {
  ...
  var newReviews = [...props.reviews];

  newReviews.forEach(review => {
    props.reviewSelections.forEach(element => {
      if(String(element.filmId) === String(review.filmId) && String(element.reviewerId) === String(review.reviewerId)){
        if(element.status === "deleted"){
          newReviews.pop(review)
        } 
        if(element.status === "added"){
          newReviews.push(review)
        }
        if(element.status === "updated"){
          var index = newReviews.findIndex(x => String(x.filmId) === String(element.filmId) && String(x.reviewerId) === String(element.reviewerId));
          if(index !== -1)
            newReviews[index] = { filmId: review.filmId, reviewerId: review.reviewerId, completed: element.completed, reviewDate: element.reviewDate, rating: element.rating, review: element.review };
        }
      }
    });
  });
      ...
      <tbody>
        {
          newReviews.map((review) =>
            <FilmReviewRow reviewData={review} filmData={props.film} key={review.reviewerId} id={review.reviewerId} 
              deleteReview={props.deleteReview} updateReview={props.updateReview} reviewSelections={props.reviewSelections}/>
          )
        }
