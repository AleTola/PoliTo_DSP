...
function ReviewLayout(props) {
  ...
  useEffect(() => {
    if (dirty) {
      ...
      API.getAllPublicFilmsToReview()
      .then(films => {
        props.setSubscribedReviewTopics(currentArray => {
          var newArray = [...currentArray]
          for (var i = 0; i < films.length; i++) {
            const topic = "review/" + String(films[i].id) + "/#";
            var index = newArray.findIndex(x => x == topic);
            if(index === -1){
              newArray.push(topic)
              props.client.subscribe("review/" + String(films[i].id) + "/#", { qos: 0 ,retain:true});
            } 
          }
          return newArray;
        });
  ...
  const refreshReviews = (filmId, pageNumber) => {
    ...
    return (
      <>
        ...
        <FilmReviewTable reviews={reviews} setReview={setReviews} film={film} reviewSelections={props.reviewSelections} 
