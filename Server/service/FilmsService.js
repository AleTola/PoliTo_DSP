      ...
      db.run(sql, [film.title, owner, film.private, film.watchDate, film.rating, film.favorite], function(err) {
          if (err) {
              reject(err);
                ...
                // Creation of a new MQTT message for the created film, if public
                if(!film.private){
                    ...
                    if (err)
                        ...
                        db.run(sql4, [filmId], (err) => {
                            if (err)
                                ...
                                /*
                                var message = new MQTTReviewMessage("deleted", null, null, null, null, null);
                                mqtt.publishReviewMessage(filmId, "#", message);
                                */

                                //if the film was active for the user, inform the clients that it is not active anymore because it has been deleted
                                if(activeReviewer != null)
                                {
                                    var oldMessage = WebSocket.getMessage(activeReviewer);
                                    var updateMessage = new WSMessage('update', parseInt(activeReviewer), oldMessage.userName, null, null);
...
// ------------------------------------- NEW -------------------------------------
/**
 * Retrieve ALL the public films for which the user has received a review invitation
 * 
 * Input: 
 * - req: the request of the user
 * Output:
 ...
 * 
 **/
exports.getAllInvitedFilms = function(reviewerId) {
    return new Promise((resolve, reject) => {
      const sql = "SELECT f.id as fid, f.title, f.owner, f.private, f.watchDate, f.rating, f.favorite FROM films f, reviews r WHERE  f.private = 0 AND f.id = r.filmId AND r.reviewerId = ?"
