...
// -------------------------------- NEW --------------------------------
  module.exports.getAllInvitedFilms = function getAllInvitedFilms (req, res, next) {
    Films.getAllInvitedFilms(String(req.user.id))
          .then(function(response) {
              utils.writeJson(res, {films: response}, 200); 
          })
          .catch(function(response) {
              utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': response }], }, 500);
          });
    ...
    Films.getSinglePrivateFilm(req.params.filmId, req.user.id)
          ...
          .catch(function(response) {
              if(response == 403){
                  ...
                  utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The film does not exist 1.' }], }, 404);
              }
              else {
                  utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': response }], }, 500);
              }
...
module.exports.updateSinglePrivateFilm = function updateSinglePrivateFilm (req, res, next) {
    ...
    .catch(function(response) {
        if(response == 403){
            ...
            utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The film does not exist 2.' }], }, 404);
        }
  ...
  Films.deleteSinglePrivateFilm(req.params.filmId, req.user.id)
        ...
        .catch(function(response) {S
            if(response == 403){
                ...
                utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The film does not exist 3.' }], }, 404);
            }
            else if (response == 409){
                utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The visibility of the film cannot be changed.' }], }, 409);
            }
...
module.exports.getSinglePublicFilm = function getSinglePublicFilm (req, res, next) {
    ...
    .catch(function(response) {
        if (response == 404){
            utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The film does not exist 4.' }], }, 404);
        }
        else {
            utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': response }], }, 500);
        }
...
module.exports.updateSinglePublicFilm = function updateSinglePublicFilm (req, res, next) {
    ...
    .catch(function(response) {
        if(response == 403){
            ...
            utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The film does not exist 5.' }], }, 404);
        }
        else {
            utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': response }], }, 500);
        }
    ...
    Films.deleteSinglePublicFilm(req.params.filmId, req.user.id)
          ...
          .catch(function(response) {S
              if(response == 403){
                  ...
                  utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The film does not exist 6.' }], }, 404);
              }
              else {
                  utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': response }], }, 500);
              }
